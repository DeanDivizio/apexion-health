...
import { deleteFoodPresetAction } from "@/actions/nutrition";
import { useToast } from "@/hooks/use-toast";
import type { FoodPresetView } from "@/lib/nutrition";

interface PresetsListProps {
  initialPresets: FoodPresetView[];
}

export function PresetsList({ initialPresets }: PresetsListProps) {
  const { toast } = useToast();
  const [presets, setPresets] = React.useState(initialPresets);
  const [builderOpen, setBuilderOpen] = React.useState(false);
  const [editingPreset, setEditingPreset] = React.useState<FoodPresetView | null>(null);
  const [deleteId, setDeleteId] = React.useState<string | null>(null);
  const [deleting, setDeleting] = React.useState(false);

  function handleEdit(preset: FoodPresetView) {
  ...
  function handleSaved(saved: FoodPresetView) {
    setPresets((prev) => {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await deleteFoodPresetAction(deleteId);
    ...
    <div className="space-y-4">
      ...
      {presets.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-12">
          No presets yet. Create one to quickly log your go-to meals.
        </p>
      )}
